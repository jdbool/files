...
app.post(
	...
	async (req, res) => {
		...
		if (
			token.allowedBytes &&
			upload.size + token.uploadedBytes > token.allowedBytes
		) {
			problem(req, res, {
				status: 403,
				title: 'Forbidden',
		...
		try {
			...
			while (true) {
				...
				if (!(await File.exists({ _id: id }))) break;
			}
			...
			console.log(
				chalk.green('[Upload] ') +
					`${file._id} - ${file.type} by ${file.ip} (${tokenID.substr(
						0,
						8
					)}...)`
			);
	...
	async (req, res) => {
		...
		if (!req.get('Range')) {
			if (
				...
				console.log(
					chalk.cyan('[Bot Hit] ') +
						`${file._id} - ${req.ip} - ${req.get('User-Agent')}`
				);
			} else {
...
app.get(
	...
	async (req, res) => {
		...
		try {
			...
			console.log(
				chalk.red('[Delete] ') + `${file._id}, unable to erase data\n\t${err}`
			);
		}
	}
);
